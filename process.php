...
for ($i = 0; $i < count($pp_config); $i++) {
    if (!isset($_POST[$get_keys[$i]]) || $_POST[$get_keys[$i]] != $pp_config[$pp_keys[$i]]) {
        header("location:index.php");
        exit;
    }
}
...
for ($i = 1; $i < 11; $i++) {
    ...
    if (!is_numeric($_POST['item_number_' . $i]) || (int)$_POST['item_number_' . $i] != (int)$itemData['item_id']) {
        $invalid = true;
    }

    if (!isset($_POST['item_name_' . $i]) || $_POST['item_name_' . $i] != $itemData['item_name']) {
        $invalid = true;
    }

    if (!isset($_POST['quantity_' . $i]) || !is_numeric($_POST['quantity_' . $i]) || (int)$_POST['quantity_' . $i] <= 0) {
        $invalid = true;
    }

    if (!isset($_POST['amount_' . $i]) || (float)$_POST['amount_' . $i] != (float)$itemData['item_price']) {
        $invalid = true;
    }
}

// if purchase is invalid, exit
if ($invalid) {
...
$data = array(
    'custom' => strtolower(cleanString($_POST['custom']))
);
...
<body>
    <div class="container text-center" style="width:750px;padding-top:50px;padding-bottom:50px;margin-top:150px;">
        ...
        <p>Thank you, <?php echo cleanString($_COOKIE['store_user']); ?></p>
